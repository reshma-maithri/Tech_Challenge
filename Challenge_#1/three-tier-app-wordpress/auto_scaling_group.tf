...
resource "google_compute_autoscaler" "wordpress" {
     depends_on = [
  ...
  zone   = var.zone
  target = google_compute_instance_group_manager.wp-igm.self_link
...
resource "google_compute_instance_template" "compute-engine" {
  ...
  network_interface {
    network = google_compute_network.vpc-network-wp.id
    access_config {
